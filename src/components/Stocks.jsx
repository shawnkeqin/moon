...
const Stocks = () => {
  ...
  const dates = ["today", "yesterday", "this_week", "last_week", "this_month", "last_month"];
    ...
    <>
      ...
      <Row gutter={[32, 32]} className="crypto-card-container">
        {wsbStocks?.wsb_stocks?.map((stockData) => (
          <Col xs={24} sm={12} lg={6} className="crypto-card" key={stockData.stock_detail.id}>
              <Card
                title={`${stockData.ticker}`}
                extra={
                  <Typography>Price: ${stockData.stock_detail.price}</Typography>
                }
              >
